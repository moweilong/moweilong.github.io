---
date: 2025-09-11 18:01:53
title: go位运算
permalink: /code/go/nfeit
categories:
  - Go
tags:
  - Go
...
示例：
若程序中多次使用 `bits.LeadingZeros32` 判断前导零，PGO 会将其优化为更高效的机器码，提升执行速度。
...
- 注意整数类型：`math/bits` 函数的参数和返回值均为无符号整数（如 `uint32`、`uint64`），使用时需注意类型转换。
- 结合文档学习：官方文档（[pkg.go.dev/math/bits](pkg.go.dev/math/bits)）提供了详细的函数说明和示例，建议结合学习。
